...
- **deliberation-empirica**: wraps Empirica's `usePlayer()`, `useGame()`, `usePlayers()`, `useStageTimer()` hooks. `resolve` calls `.get()` on the appropriate Empirica objects, `save` calls `.set()`, `getElapsedTime` reads the stage timer during game stages and falls back to `Date.now()` during intro/exit.
- **VS Code preview**: provides static mock data for `resolve`, no-ops for `save`, and a controllable clock for `getElapsedTime`.
- **A future platform**: implements the same interface against its own data layer.
